...
/** The conventional companion timeline track name for a media rendition: `<rendition>.timeline`. */
export function trackName(rendition: string): string {
	return `${rendition}.timeline`;
}
...
export class Producer {
	 ...
	 * pairing of a media timestamp `pts` (microseconds) with its wall-clock time `unixMillis` (Unix
	 * epoch milliseconds, defaulting to now). Stored as the extrapolated wall-clock time of pts 0,
	 * the single value the catalog `wall` field carries: in this timeline's timescale, measured from
	 * the moq epoch ({@link Catalog.MOQ_EPOCH_UNIX_MILLIS}, 2020). Throws if `unixMillis` predates
	 * the moq epoch (unrepresentable).
	 */
	setWall(pts: Time.Micro, unixMillis: number = Date.now()): void {
